---
title: "210522_HeatmapsOfRelevantGenes"
...
p1 <- pheatmap(topgenes_selection_matrix,scale = "row",cluster_cols = FALSE,cluster_rows = FALSE, silent =TRUE)

pdf(file = filename, width = 5, height = 3)
  p1
  dev.off()
...
#save heatmap
p2 <- pheatmap(Mek18_Matrix, scale = "row", cluster_cols = FALSE, silent = TRUE)
...
# Save the heatmap
p4 <- pheatmap(wagle_matrix,
         scale = "row",
         cluster_cols = FALSE,
         silent = TRUE)

filename = paste("../03_output/", today, "_4_WagleMek_heatmap.pdf", sep = "")
pdf(file = filename, width = 5, height = 3.5)
  p4
  dev.off()
...
# save heatmap
p6 <- pheatmap(as.matrix(lobodaUp_matrix),
         scale = "row",
         cluster_cols = FALSE,
         silent = TRUE)
...
xlabelnames <- colnames(Pvalue_df[c(4,2,1,3,5),1:3])
ylabelnames <- rownames(Pvalue_df[c(4,2,1,3,5),1:3])
...
depletion_matrix$PubList <- as.factor(depletion_matrix$PubList)
colnames(depletion_matrix)[3] <- "neglogpvalue"
...
plot1 <- ggplot(long_Pvalues, aes(x=RNAseq_num, y=PubList, color=log(foldenrich))) + 
  ...
  scale_x_discrete(name = "Gene sets (this study)", labels=xlabelnames, limits = factor(c(1, 2, 3))) +
...
plot2 <- ggplot(depletion_matrix, aes(x=RNAseq_num, y=PubList, color=log(folddeplete))) + 
  geom_point(aes(size=neglogpvalue)) +
  scale_colour_gradient2(midpoint = 1, low = "cyan", mid = "blue", high = "navy", limits = c(-3, 3)) +
  scale_size(range = c(1, 20)) +
  scale_x_discrete(name = "Gene sets (this study)", labels=xlabelnames, limits = factor(c(1, 2, 3))) +
  scale_y_discrete(name = "Published gene signature sets", labels=rev(ylabelnames), limits = c("5", "4", "3", "2", "1")) +
  theme_bw() +
